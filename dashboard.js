...
function renderMedicationsCard(person) {
  ...
  return `<div class="card">
    <div class="card-header"><span class="card-title">Medikamente</span></div>
...
function renderVaccinationsCard(person) {
  ...
  if (!items.length) return '';
  return `<div class="card">
    <div class="card-header"><span class="card-title">Impfungen</span></div>
...
function renderAllergiesCard(person) {
  ...
  if (!items.length) return '';
  return `<div class="card">
    <div class="card-header"><span class="card-title">Allergien</span></div>
...
function renderAlerts(alerts) {
  ...
  return alerts.map(a=>`<div class="alert ${a.status==='overdue'?'alert-danger':'alert-warning'}">
    <span class="alert-icon">${a.status==='overdue'?'⚠️':'⏰'}</span>
    <div class="alert-body">
      <strong>${esc(a.checkup.name)}</strong>
      <span>${a.label}${a.lastDate?` — Zuletzt: ${fmtDate(a.lastDate)}`:' — Noch nie'}</span>
    </div>
  </div>`).join('');
}
